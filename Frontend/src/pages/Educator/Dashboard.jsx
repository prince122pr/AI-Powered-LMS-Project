...
const Dashboard = () => {
    ...
    const fetchCourses = async () => {
      try {
        ...
        const response = await axios.get(
          `${backendBaseURL}/course/creator-courses`,
          {
  ...
  useEffect(() => {
    setIsVisible(true);
       dispatch(getAllCourses()); // jab dashboard open ho tab latest courses le aao
  }, [dispatch]);
  ...
  const handleDeleteCourse = async (courseId) => {
    if (window.confirm("Are you sure you want to delete this course?")) {
      try {
        await axios.delete(
          `${backendBaseURL}/course/delete-course/${courseId}`,
          {
            withCredentials: true,
          }
        );
        ...
        dispatch(
          setCreatorCourseData(
            creatorCourses.filter((course) => course._id !== courseId)
          )
        );
      } catch (err) {
  ...
  return (
    <div className="min-h-screen pt-10 bg-gradient-to-b from-gray-900 via-gray-800 to-gray-900 text-white overflow-hidden">
      {/* Header Section */}
      <div
        className={`relative py-12 mt-6 px-4 sm:px-6 lg:px-8 mb-10 transition-all duration-1000 ${
          isVisible
        ...
        <div className="relative z-10 max-w-7xl mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-center">
            <div className="mb-6 md:mb-0">
              ...
              <button
                onClick={() => navigate("/")}
                className="cursor-pointer px-4 py-2 bg-gradient-to-r from-orange-500 to-red-600 
      ...
      <div className="mx-auto px-4 sm:px-6 lg:px-8 pb-16">
        {/* Stats Cards */}
        <div
          className={`grid grid-cols-1 md:grid-cols-4 gap-6 mb-10 transition-all duration-1000 delay-300 ${
            isVisible
              ? "opacity-100 transform translate-y-0"
              : "opacity-0 transform translate-y-10"
          }`}
        >
          {/* Cards (same as your UI) */}
          {/* Total Courses */}
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-xl shadow-xl border border-gray-700 hover:border-orange-500/50 transition-all duration-300 transform hover:-translate-y-1 group">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-gray-400 text-sm font-medium mb-1 group-hover:text-orange-300 transition-colors">
                  Total Courses
                </h3>
                <p className="text-4xl font-[f2] font-bold text-white group-hover:text-transparent group-hover:bg-clip-text group-hover:bg-gradient-to-r group-hover:from-orange-400 group-hover:to-red-500 transition-all">
                  {totalCourses}
                </p>
              </div>
          ...
          {/* Published, Students, Earning cards same UI */}
          <div className="bg-gradient-to-br from-gray-800 to-gray-900 p-6 rounded-xl shadow-xl border border-gray-700 hover:border-orange-500/50 transition-all duration-300 transform hover:-translate-y-1 group">
            <div className="flex items-start justify-between">
              <div>
                <h3 className="text-gray-400 text-sm font-medium mb-1 group-hover:text-orange-300 transition-colors">
        ...
        {/* Courses List */}
        <div
          className={`bg-gradient-to-br from-gray-800 to-gray-900 rounded-xl shadow-xl border border-gray-700 overflow-hidden transition-all duration-1000 delay-500 ${
            isVisible
              ? "opacity-100 transform translate-y-0"
              : "opacity-0 transform translate-y-10"
          }`}
